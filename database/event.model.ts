...
/**
 * Normalises a date string to ISO 8601 (YYYY-MM-DD) computed in UTC.
 *
 * Why " UTC" is appended:
 *   new Date("May 20, 2026") is parsed as *local* midnight, so on machines
 *   west of UTC (e.g. UTC-5) toISOString() returns "2026-05-19T05:00:00.000Z"
 *   — shifting the calendar day back by one.
 *   Appending " UTC" forces the JS engine to treat the input as UTC midnight,
 *   so toISOString() always yields the correct YYYY-MM-DD regardless of the
 *   server's timezone.
 *
 * Accepts human-readable strings like "May 20, 2026" or "2026-05-20".
 * Throws if the value cannot be parsed.
 */
function normaliseDate(raw: string): string {
  // Trim first; if the string already ends with "Z" or "+HH:MM" it already
  // carries timezone info — don't double-append " UTC".
  const trimmed = raw.trim();
  const hasTimezone = /Z$|[+-]\d{2}:\d{2}$/.test(trimmed);
  const input = hasTimezone ? trimmed : `${trimmed} UTC`;
  ...
  // toISOString() is always UTC, so splitting on "T" gives the correct date.
...
eventSchema.pre<IEventDocument>("save", function (this: IEventDocument) {
  ...
  // Normalise date to UTC-based ISO format (YYYY-MM-DD) on every save.
